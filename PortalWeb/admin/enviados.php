...
<?php
$con=conectarse();
$_pagi_sql="SELECT orden.OrdenID,orden.UsuarioID,orden.OrdenEstado,orden.OrdenFecha, usuarios.Correo FROM usuarios,orden where orden.OrdenEstado=1 and orden.UsuarioID=usuarios.UsuarioID";
$_pagi_cuantos=10;
...
echo "<td class='".$estilo."'><input type=checkbox name='c[".$fila[OrdenID]."]' checked></td>\n";
echo "</tr>";
    }
...
if (!empty($_POST['d'])) {
	...
	echo "location.href='enviados.php';";
	echo "</SCRIPT>";
}
if (!empty($_POST['c'])) {
    $con=conectarse();
    $aLista=array_keys($_POST['c']);
    $sql="UPDATE orden SET OrdenEstado=0 where OrdenID IN (".implode(',',$aLista).")";
    mysql_query($sql,$con);
    mysql_close($con);
    echo "<SCRIPT LANGUAGE=\"JavaScript\">\n";
	echo "location.href='enviados.php';";
